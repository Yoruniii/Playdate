...
public class HappySunToy extends BlockWithEntity{

    public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;

    public static BooleanProperty POSSESSED = BooleanProperty.of("possessed");

    public HappySunToy(Settings settings) {
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HappySunToyEntity(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, Playdate.HAPPY_SUN_TOY_ENTITY, (world1, pos, state1, be) -> HappySunToyEntity.tick(world1, pos, state1, be));
    }
