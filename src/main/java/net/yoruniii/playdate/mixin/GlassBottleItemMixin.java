package net.yoruniii.playdate.mixin;
...
public abstract class GlassBottleItemMixin extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!list.isEmpty()) {
            ...
            HitResult areaEffectCloudEntity = raycast(world, user, RaycastContext.FluidHandling.SOURCE_ONLY);
            if (areaEffectCloudEntity.getType() == HitResult.Type.MISS) {
                return TypedActionResult.pass(itemStack);
            } else {
                if (areaEffectCloudEntity.getType() == HitResult.Type.BLOCK) {
                    BlockPos blockPos = ((BlockHitResult)areaEffectCloudEntity).getBlockPos();
                    ...
                    {
                        world.playSound(user, user.getX(), user.getY(), user.getZ(), SoundEvents.BLOCK_SOUL_SAND_BREAK, SoundCategory.NEUTRAL, 1, 1);
                        world.setBlockState(blockPos, Blocks.SOUL_SOIL.getDefaultState());
                        world.playSound(user, blockPos, SoundEvents.AMBIENT_SOUL_SAND_VALLEY_ADDITIONS, SoundCategory.NEUTRAL, 1, 1);
                        return TypedActionResult.success(this.fill(itemStack, user, new ItemStack(ModItems.BOTTLED_SOUL)));
                    }
                }
